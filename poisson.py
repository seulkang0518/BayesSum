...
# ============================================================
#                Polynomial kernel (degree-sensitive)
# ============================================================
...
_S2 = _stirling2_table(8)  # up to degree 8
...
def poly_params_from_nodes(X, *, c: float, degree: int):
    """
    Per-seed params:
      μ = sample mean of nodes (centering)
      α = (1 - c) / Var_nodes  (so E[k(x,x)] ≈ amp when degree=1)
      α_bnd = 1.0  (NO (1+c)^(-degree) squash — preserves degree differences)
    """
    X = jnp.asarray(X, dtype=jnp.float64)
    mu = float(jnp.mean(X))
    var = float(jnp.var(X) + 1e-12)
    alpha = (1.0 - c) / var
    alpha_bnd = 1.0
    return mu, alpha, alpha_bnd

def polynomial_kernel_with_params(X1, X2, *, degree: int, c: float, amp: float,
                                  mu: float, alpha: float, alpha_bnd: float):
    ...
    K = amp * jnp.power(base, degree) * alpha_bnd
    return jnp.nan_to_num(K, posinf=1e300, neginf=-1e300)

def poly_kernel_embedding_poisson_with_params(lambda_, X, *, degree: int, c: float,
                                              amp: float, mu: float, alpha: float, alpha_bnd: float):
    X = jnp.atleast_1d(X).astype(jnp.float64)
    d = int(degree)
    powers = jnp.arange(d + 1, dtype=jnp.float64)

    m_raw = _poisson_raw_moments(lambda_, d)
    B = jnp.array([[_binom_int(r, k) for k in range(d + 1)] for r in range(d + 1)],
                  dtype=jnp.float64)
    MU = jnp.array([((-mu) ** (r - k) if k <= r else 0.0)
                    for r in range(d + 1) for k in range(d + 1)],
                   dtype=jnp.float64).reshape(d + 1, d + 1)
    c_mom = (B * MU) @ m_raw

    xpow = (X - mu)[:, None] ** powers[None, :]
    binoms = jnp.array([_binom_int(d, int(p)) for p in powers], dtype=jnp.float64)
    coeff = binoms * (c ** (d - powers)) * (alpha ** powers)
    z = amp * (xpow * (coeff * c_mom)[None, :]).sum(axis=1)
    return jnp.nan_to_num(z) * alpha_bnd

def poly_double_integral_poisson_with_params(lambda_, *, degree: int, c: float,
                                             amp: float, mu: float, alpha: float, alpha_bnd: float):
    d = int(degree)
    powers = jnp.arange(d + 1, dtype=jnp.float64)
    ...
    return float(J * alpha_bnd)

def bayesian_cubature_poisson_poly(X, f_vals, lambda_, degree: int,
                                   c: float = 0.3, amp: float = 1.0, jitter=1e-4):
    """BC mean/var using per-seed centered & degree-sensitive polynomial kernel."""
    X = jnp.atleast_1d(X).astype(jnp.float64)
    f_vals = jnp.atleast_1d(f_vals).astype(jnp.float64)
    n = X.shape[0]

    mu_ps, alpha_ps, alpha_bnd = poly_params_from_nodes(X, c=c, degree=degree)
    K = polynomial_kernel_with_params(X, X, degree=degree, c=c, amp=amp,
                                      mu=mu_ps, alpha=alpha_ps, alpha_bnd=alpha_bnd)

    ridge = (max(jitter, 1e-5) * amp + 1e-8) * jnp.eye(n)   # a touch stronger for stability
    K = K + ridge

    L, lower = cho_factor(K, overwrite_a=False, check_finite=False)

    z = poly_kernel_embedding_poisson_with_params(lambda_, X, degree=degree, c=c, amp=amp,
                                                  mu=mu_ps, alpha=alpha_ps, alpha_bnd=alpha_bnd).reshape(n, 1)
    ...
    mean = (z.T @ K_inv_f)[0, 0]
    J = poly_double_integral_poisson_with_params(lambda_, degree=degree, c=c, amp=amp,
                                                 mu=mu_ps, alpha=alpha_ps, alpha_bnd=alpha_bnd)
    var = J - (z.T @ K_inv_z)[0, 0]
    return float(jnp.nan_to_num(mean)), float(jnp.maximum(jnp.nan_to_num(var), 1e-30))

def poly_diagnostics(X, lambda_, degree, c, amp, jitter):
    mu_ps, alpha_ps, alpha_bnd = poly_params_from_nodes(X, c=c, degree=degree)
    K = polynomial_kernel_with_params(X, X, degree=degree, c=c, amp=amp,
                                      mu=mu_ps, alpha=alpha_ps, alpha_bnd=alpha_bnd)
    K = K + (jitter * amp + 1e-10) * jnp.eye(K.shape[0])

    evals = jnp.linalg.eigvalsh((K + K.T) * 0.5)
    lam_min = jnp.min(evals)
    lam_max = jnp.max(evals)
    cond = lam_max / jnp.maximum(lam_min, 1e-300)

    L, lower = cho_factor(K, overwrite_a=False, check_finite=False)
    z = poly_kernel_embedding_poisson_with_params(
        lambda_, X, degree=degree, c=c, amp=amp,
        mu=mu_ps, alpha=alpha_ps, alpha_bnd=alpha_bnd
    ).reshape(-1, 1)
    w = cho_solve((L, lower), z, check_finite=False)

    w_norm = float(jnp.linalg.norm(w))
    w_max  = float(jnp.max(jnp.abs(w)))
    return float(cond), float(lam_min), w_norm, w_max
...
def run_experiment(f, n_vals, lambda_, r, xmax, seed, all_states, experiment_type, amp, poly_deg=None, poly_c=0.3):
    ...
    for n in n_vals:
        ...
        if experiment_type == "bq_poly":
            ...
            # --- oversample ---
            oversample = target_n * 5
            X_proposed = random.poisson(subkey, lam=lambda_, shape=(oversample,))
            X_unique = jnp.unique(X_proposed)

            # --- if still not enough points, recursively resample additional ones ---
            while X_unique.shape[0] < 2*target_n/3:
                subkey, extra_key = random.split(subkey)
                extra = random.poisson(extra_key, lam=lambda_, shape=(oversample,))
                X_unique = jnp.unique(jnp.concatenate([X_unique, extra]))

            # --- pick exactly n unique points ---
            X_eval = X_unique[:target_n]
            f_vals_eval = f(X_eval)
            ...
            est_mean, _ = rr_poisson_tail_estimator(subkey, lambda_, lam_star=20)
...
def run_multiple_seeds(f, n_vals, lambda_, r, xmax, num_seeds, all_states, experiment_type, amp=1.0, poly_deg=None, poly_c=0.3):
    all_errors_across_seeds = []

    for seed in range(num_seeds):
        print(f"\n--- Running {experiment_type}, Seed {seed+1}/{num_seeds} ---")
        result = run_experiment(f, n_vals, lambda_, r, xmax, seed, all_states, experiment_type, amp=amp, poly_deg=poly_deg, poly_c=poly_c)
...
def plot_results_poly(n_vals, all_results, save_path="results"):
    os.makedirs(save_path, exist_ok=True)
    styles = {
        'Poly d=2 (Active)': {'color': '#E75480', 'marker': 'D', 'label': 'Degree 2'},
        'Poly d=5 (Active)': {'color': '#FF69B4', 'marker': 'D', 'label': 'Degree 5'},
        'Poly d=8 (Active)': {'color': '#800080', 'marker': 'D', 'label': 'Degree 8'},
    }
    plt.figure(figsize=(10, 6))
    handles, labels = [], []
    for name in [m for m in styles.keys() if m in all_results]:
    ...
    plt.grid(True, which='major', linestyle='--', linewidth=0.5)
    plt.tight_layout()
    plt.legend(handles, labels, loc='upper right', fontsize=22, frameon=False, ncol=1, bbox_to_anchor=(0.98, 0.98))
    plt.savefig(os.path.join(save_path, "poisson_abs_err_poly.pdf"), format='pdf')
    plt.close()
...
def main():
    ...
    all_results = {}
    # all_results['Active BayesSum'] = run_multiple_seeds(f, n_vals, lambda_val, r, xmax, num_seeds, all_states, "bq_bo", 1.0)
    # all_results['BayesSum'] = run_multiple_seeds(f, n_vals, lambda_val, r, xmax, num_seeds, all_states, "bq_random", 1.0)
    # all_results['MC'] = run_multiple_seeds(f, n_vals, lambda_val, r, xmax, num_seeds, all_states, "mc", 1.0)
    # all_results['RR'] = run_multiple_seeds(f, n_vals, lambda_val, r, xmax, num_seeds, all_states, "rr_pois", 1.0)
    # all_results['IS'] = run_multiple_seeds(f, n_vals, lambda_val, r, xmax, num_seeds, all_states, "is", 1.0)
    # all_results['SS'] = run_multiple_seeds(f, n_vals, lambda_val, r, xmax, num_seeds, all_states, "ss", 1.0)

    # save_results_portable("poisson_BS.npz", n_vals, all_results)
    # print("poisson.npz")
    
    n_vals_loaded_BS, all_results_loaded_BS = load_results_portable("poisson_BS.npz")
    n_vals_loaded, all_results_loaded = load_results_portable("poisson.npz")
    all_results_loaded['BayesSum'] = all_results_loaded_BS['BayesSum']
    # print(all_results_loaded)
    plot_results(n_vals_loaded, all_results_loaded)

    # methods = ["MC", "BayesSum", "Active BayesSum", "RR", "SS", "IS"]

    # for m in methods:
    #     alpha, a, b = estimate_alpha(n_vals_loaded, all_results_loaded[m]["mean_abs_error"], min_index=1)
    #     print(f"{m}: α ≈ {alpha:.3f}")

    # all_results = {}
    # for deg in [2, 5, 8]:
    #     label = f"Poly d={deg} (Active)"
    #     all_results[label] = run_multiple_seeds(
    #         f, n_vals, lambda_val, r, xmax, num_seeds, all_states,
    #         experiment_type="bq_poly", amp=1.0, poly_deg=deg, poly_c=0.3  # c=0.3 to keep degree effects visible
    #     )

    # save_results_portable("poisson_poly.npz", n_vals, all_results)
    # print("poisson_poly.npz")

    # n_vals_loaded, all_results_loaded = load_results_portable("poisson_poly.npz")
    # print("plotting keys:", list(all_results_loaded.keys()))
