...
def run_experiments(
    ...
    for seed in seeds:
        ...
        for n_model in n_models:
            ...
            # IMPORTANT: pass *fresh* arrays so later runs don’t mutate shared memory
...
def plot_ksd_nll(ksd_mc, ksd_bq, nll_mc, nll_bq, n_models, d, p):
    ...
    ax1.plot(n_models, ksd_bq, 'b-s', label='KSD$^2$ (BayesSum)')
    ax1.set_yscale('log')
...
if __name__ == "__main__":
    ...
    # initializer that returns the SAME (h0,J0) for a given seed
    def init_provider(seed):
        return init_params_from_seed(seed, d, q)

    # OPTIONAL: if you also want the same initial Gibbs state across MC vs BQ
    # for each (seed, n_model), plug this in. Otherwise set to None.
    def gibbs_x0_provider(seed, n_model):
        # tag by n_model to keep different model-sizes distinct but shared across MC/BQ
        return init_gibbs_state_from_seed(seed, d, q, tag=n_model)
