...
def all_states_cached(d):
    return jnp.array(list(product([0., 1.], repeat=d)))

def run_experiment(f, n_vals, lambda_, d, L, seed, use_bo):
    key = jax.random.PRNGKey(seed)
    J = J_matrix(L)
    all_states = all_states_cached(d)
    true_vals = f_batch(all_states, J, beta)
    true_expectation = jnp.mean(true_vals)
    ...
    for n in n_vals:
        ...
        if use_bo:
            ...
            key, subkey = jax.random.split(key)
            idx = jax.random.choice(subkey, len(candidates))
            x0 = candidates[idx]
            X = [x0]
            y = [f_single(x0, J, beta)]
            for _ in range(1, n):
                key, subkey = jax.random.split(key)
                x_next = compute_kg(jnp.array(X), jnp.array(y).reshape(-1, 1), candidates, lambda_, d, key, num_samples = 10)
                # x_next = compute_max_variance(jnp.array(X), jnp.array(y).reshape(-1, 1), candidates, lambda_, d)
                # x_next = compute_ei_bq(jnp.array(X), candidates, lambda_, d)
                # x_next = compute_ei(jnp.array(X), jnp.array(y).reshape(-1, 1),candidates, lambda_, d)
        ...
        print(f"n={n}, Time={elapsed:.3f}s, BMC err={float(jnp.abs(mu_bmc - true_expectation)):.10f}")
        print(f"n={n}, MC   err={float(jnp.abs(mu_mc - true_expectation)):.10f}")
...
def run_multiple_seeds(f, n_vals, lambda_, d, L, num_seeds, use_bo):
    bmc_all = []
    mc_all = []
    times_all = []
    ...
    for seed in range(num_seeds):
        print(f"\n--- Seed {seed} ---")
        result = run_experiment(f, n_vals, lambda_, d, L, seed, use_bo)
...
def main():
    ...
    num_seeds = 10
    use_bo = True  # Try False to test random BMC

    results = run_multiple_seeds(f_single, n_vals, lambda_, d, L, num_seeds, use_bo)
