...
# ====================
M_SUB = 1000  # fixed Python int for subsampling the pool (avoids JAX tracing issues)
...
def median_heuristic_gamma(samples, n_subset=500):
    ...
    gamma = 1.0 / (2.0 * jnp.clip(median_sq, 1e-8, None))
    return gamma
...
def run_single_experiment(seed):
    ...
    for step in range(num_steps):
        ...
        params, opt_state, loss, log_Z = update_step(
            ...
            weight_decay=weight_decay, run_bq=True,
        )
        if step % 20 == 0:
