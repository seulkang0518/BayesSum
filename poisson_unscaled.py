...
@jit
def estimate_and_logml(K, y, z_scaled, integral_sq_norm, jitter=1e-8):
    n = K.shape[0]
    C = K + jitter * jnp.eye(n)
    y = y.reshape(-1)
    L, lower = cho_factor(C, lower=True)
    
    C_inv_y = cho_solve((L, lower), y)
    bq_mean = jnp.dot(z_scaled.reshape(1, -1), C_inv_y)[0]
    ...
    C_inv_z = cho_solve((L, lower), z_scaled)
    bq_var = integral_sq_norm - jnp.dot(z_scaled.T, C_inv_z)
    
    return bq_mean, bq_var, logml

# #### UPDATED: Main tuning function now uses a gradient-based optimizer ####
def tune_and_estimate_poisson_bq(X, f_vals, lambda_, xmax):
    # Base (unscaled) kernel embedding and integral
    z_base = kernel_embedding_poisson(lambda_, X, xmax).reshape(-1, 1)
    integral_base = double_integral_poisson(lambda_, xmax)

    # Heuristic for a good starting guess for the optimizer
    amplitude_initial = jnp.maximum(jnp.var(f_vals), 1e-4)
    initial_params = jnp.log(amplitude_initial) # Optimize in log-space
    
    # Define the objective function (negative log-ML)
    @jit
    def nll(log_amplitude):
        amplitude = jnp.exp(log_amplitude)
        K = brownian_kernel(X, X, amplitude)
        z_scaled = amplitude * z_base
        integral_sq_norm = amplitude * integral_base
        _, _, logml = estimate_and_logml(K, f_vals, z_scaled, integral_sq_norm)
        return -logml

    # Use a gradient-based optimizer to find the best amplitude
    result = minimize(
        fun=nll,
        x0=initial_params,
        method='L-BFGS-B',
        jac=jit(jax.grad(nll)) # Provide the gradient automatically with JAX
    )

    # Extract the optimal amplitude
    amplitude_opt = jnp.exp(result.x[0])
    
    # Now, calculate the final BQ estimate with the optimal amplitude
    K_opt = brownian_kernel(X, X, amplitude_opt)
    z_opt_scaled = amplitude_opt * z_base
    integral_sq_norm_opt = amplitude_opt * integral_base
    
    mean, var, _ = estimate_and_logml(K_opt, f_vals, z_opt_scaled, integral_sq_norm_opt)
    
    return float(np.asarray(mean)), float(np.asarray(jnp.maximum(var, 1e-30)))
...
def run_experiment(f, n_vals, lambda_, xmax, seed, all_states, experiment_type):
    ...
    for n in n_vals:
        ...
        if experiment_type == "bq_bo":
            ...
            est_mean, _ = tune_and_estimate_poisson_bq(X_eval, f_vals_eval, lambda_, xmax)
...
def run_multiple_seeds(f, n_vals, lambda_, xmax, num_seeds, all_states, experiment_type):
    all_errors_across_seeds = []
    for seed in range(num_seeds):
        print(f"\n--- Running {experiment_type}, Seed {seed+1}/{num_seeds} ---")
        result = run_experiment(f, n_vals, lambda_, xmax, seed, all_states, experiment_type)
...
def calibration(f, n, seeds, lambda_, xmax, out_png="poisson_reliability.pdf"):
    all_states = jnp.arange(0, xmax+1, dtype=jnp.int64)
    t_e = jnp.sum(f(all_states) * jax_poisson.pmf(all_states, mu=lambda_))
    mus, vars_ = [], []
    print(f"\n--- Running calibration with N={n}, Seeds={seeds} ---")
    for s in range(seeds):
        ...
        # Use the new, tuned BQ estimator
        mu, var = tune_and_estimate_poisson_bq(X, y, lambda_, xmax)
    ...
    plt.plot(np.asarray(C_nom), np.asarray(emp_cov), "o-", label=f"DBQ (Tuned), N={n}")
...
def main():
    ...
    all_results = {}
    all_results['Active DBQ'] = run_multiple_seeds(f_exp, n_vals, lambda_val, xmax, num_seeds, all_states, "bq_bo")
    all_results['DBQ'] = run_multiple_seeds(f_exp, n_vals, lambda_val, xmax, num_seeds, all_states, "bq_random")
    all_results['MC'] = run_multiple_seeds(f_exp, n_vals, lambda_val, xmax, num_seeds, all_states, "mc")
    all_results['RR'] = run_multiple_seeds(f_exp, n_vals, lambda_val, xmax, num_seeds, all_states, "rr_geom")
    
    plot_results(n_vals, all_results)

    # calibration_seeds = 200
    # n_samples = 60
    # t_true, C_nom, emp_cov, mus, vars_ = calibration(f_exp, n_samples, calibration_seeds, lambda_val, xmax)
