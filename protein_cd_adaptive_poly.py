...
def median_heuristic_lambda_hamming(samples_onehot, n_subset=500, key=None):
    ...
    # Two common choices:
    # 1) lambda = 1 / median   (so exp(-lambda * median) ~ e^-1)
    # 2) lambda = log(2) / median (median similarity ~ 0.5)
...
def logZ_exact_enumeration(h, J, J_mask, beta, L: int, q: int, chunk: int = 200_000) -> float:
    ...
    for start in range(0, total, chunk):
        ...
        # energies and chunk log-sum-exp
...
def base_exp_hamming_kernel(xf, yf, d, lam):
    dot = jnp.dot(xf, yf)   
...
@jit
def stein_kernel_pair_params(h, J, J_mask, beta, x_int, y_int, lam, _unused=None):
    d = x_int.shape[0]
    q = h.shape[1]
    ...
    xf = onehot_flat_from_int(x_int, q)  # (d*q,)
    yf = onehot_flat_from_int(y_int, q)  # (d*q,)
    k_xy = base_exp_hamming_kernel(xf, yf, d, lam)
    ...
    k_x_yinv    = vmap(lambda yf_: base_exp_hamming_kernel(xf, yf_, d, lam))(yinv_flat)
    k_xinv_y    = vmap(lambda xf_: base_exp_hamming_kernel(xf_, yf, d, lam))(xinv_flat)
    K_xinv_yinv = vmap(lambda xf_: vmap(lambda yf_: base_exp_hamming_kernel(xf_, yf_, d, lam))(yinv_flat))(xinv_flat)
...
def stein_kernel_matrix_params(h, J, J_mask, beta, X_int, lam, _unused=None):
    pair = lambda xi, xj: stein_kernel_pair_params(h, J, J_mask, beta, xi, xj, lam, None)
    return vmap(vmap(pair, in_axes=(None, 0)), in_axes=(0, None))(X_int, X_int)

def stein_bq_poly2_mu(h, J, J_mask, beta, X_int, Fmat, lam, ridge=1e-3, jitter=1e-8):

    Ks = stein_kernel_matrix_params(h, J, J_mask, beta, X_int, lam, None)
    n = Ks.shape[0]
...
# ------------------------
def cd_main(X_true_oh, X_model_oh, X_model_int, J, J_mask, h, beta, lam, lambda_J, lambda_h, run_bq):
    d, q = h.shape
    dEh_data, dEJ_data = mean_grad_U_over(X_true_oh, h, J, J_mask)

    if run_bq:
        Fmat = build_Fmat_beta_grad_raw(h, J, J_mask, X_model_oh)  # (n, d*q + d*d*q*q)
        mu_hat = stein_bq_poly2_mu(h, J, J_mask, beta, X_model_int, Fmat, lam, ridge=1e-3)
...
def cd_training(seed, d, q, p, beta, steps, l_r, n_model, n_true, lambda_h, lambda_J,
    ...
    key, k_h, k_J, k_data = jrand.split(key, 4)
    ...
    # Train/val split on integer data (keep one-hot for train gradient)
    n_val = max(1, int(val_frac * n_true))
    X_val_int   = X_true_int[-n_val:]       # (n_val, d)
    X_train_int = X_true_int[:-n_val]       # (n_train, d)
    X_train_oh  = X_true_oh[:-n_val]        # (n_train, d, q)
    ...
    for t in range(steps):
        ...
        # --- Use Gibbs negatives (recommended) ---
        X_model_int = gibbs_sample_compiled(h, J, q, beta, k_gibbs, n_model, burnin=200, thinning=2)
        X_model_oh  = jax.nn.one_hot(X_model_int, num_classes=q, dtype=jnp.float64)

        # --- Alternatively, random negatives (debugging) ---
        # X_model_int = jax.random.randint(k_gibbs, (n_model, d), minval=0, maxval=q)
        # X_model_oh = jax.nn.one_hot(X_model_int, num_classes=q, dtype=jnp.float64)
        lam = median_heuristic_lambda_hamming(X_model_oh, n_subset=500, key=None)
        grad_h, grad_J = cd_main(X_train_oh, X_model_oh, X_model_int, J, J_mask, h,
                                 beta, lam, lambda_J, lambda_h, run_bq)

        # Update
        h, J, opt_state = apply_update(h, J, grad_h, grad_J, opt_state, J_mask, tx)

        # Metrics (validation NPLL, norms)
        ...
        # Optional mini-KSD monitor (cheap-ish), using RBF median heuristic
    ...
    # Return best params (by val NPLL)
...
def plot_ksd_nll(ksd_mc, ksd_bq, nll_mc, nll_bq, n_models, d, p):
    # true_nll = _true_nll_nats(d, p)

    fig, ax1 = plt.subplots(figsize=(10.0, 6.0))

    ax1.plot(n_models, ksd_mc, 'k-o', label='KSD$^2$ (MC)')      # MC KSD^2
    ax1.plot(n_models, ksd_bq, 'b-s', label='KSD$^2$ (BayesSum)')        # BQ KSD^2
    ax1.set_yscale('log')
    ...
    ax2 = ax1.twinx()
    ax2.plot(n_models, nll_mc, 'k--o', label='NLL (MC)')       # MC NLL
    ax2.plot(n_models, nll_bq, 'b--s', label='NLL (BayesSum)')       # BQ NLL
...
if __name__ == "__main__":
    ...
    # nll_mc = [15.79306507, 15.79550544, 15.79149294, 15.79049697]
    # ksd_mc = [0.0038607,  0.00272616, 0.00314983, 0.00269069]

    # nll_mc = [15.80079335, 15.78860282, 15.78658625, 15.78535309, 15.78737204]
    # ksd_mc = [0.00225555, 0.00223991, 0.00188253, 0.00156346, 0.00136589]

    nll_mc = [15.78860282, 15.78658625, 15.78535309, 15.78737204]
    ksd_mc = [0.00223991, 0.00188253, 0.00156346, 0.00136589]


    # nll_bq = [15.81395065, 15.8003525,  15.79262471, 15.78589122, 15.78574333]
    # ksd_bq = [0.00407896, 0.00199869, 0.00174832, 0.00126697, 0.00114259]

    ksd_bq = [1.512111e-03, 1.807600e-03, 1.397320e-03, 1.242560e-03]
    nll_bq = [1.5792042e+01, 1.5787214e+01, 1.5784300e+01, 1.5784442e+01]

    # ksd_bq = [3.41194e-03, 1.512111e-03, 1.807600e-03, 1.397320e-03, 1.242560e-03]
    # nll_bq = [1.5819240e+01, 1.5792042e+01, 1.5787214e+01, 1.5784300e+01, 1.5784442e+01]
    plot_ksd_nll(ksd_mc, ksd_bq, nll_mc, nll_bq, n_models, d, p)

    # print("\nSummary:")
    # for r in results:
    #     print(f"Seed={r['seed']} | n_model={r['n_model']} | KSD^2={r['ksd2']:.3e}")


    # ksd_bq = jnp.array([3.065e-03, 2.367e-03, 2.017e-03, 1.444e-03, 1.212e-03])
    # ksd_mc = jnp.array([2.255e-03, 2.241e-03, 2.184e-03, 1.546e-03, 1.217e-03])