    ...
    for n in n_vals:
        ...
        if experiment_type == "bq_bo":
            ...
            while len(X) < n:
                ...
                x_next = compute_max_variance(jnp.array(X), unused, lambda_, xmax) if len(unused) > 0 else random.choice(key, all_states)
            ...
            est_mean, _ = bayesian_cubature_poisson(X_eval, f_vals_eval, lambda_, xmax)
...
def calibration(n, seeds, lambda_, xmax, out_png="poisson_reliability_N60.png"):
    ...
    for s in range(seeds):
        ...
        mu, var = bayesian_cubature_poisson(X, y, lambda_, xmax)
    ...
    # plt.close()
    # print(f"Poisson: true E[f(X)]={float(t_e):.6f}  |  saved {out_png}")
...
def main():
    ...
    # all_results = {}
    # all_results['Active DBQ'] = run_multiple_seeds(f, n_vals, lambda_val, xmax, num_seeds, all_states, "bq_bo")
    # all_results['DBQ'] = run_multiple_seeds(f, n_vals, lambda_val, xmax, num_seeds, all_states, "bq_random")
