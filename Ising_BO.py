...
### 1. J matrix for Ising-style interaction ###
def J_matrix(L):
    n = L * L
...
### 2. Energy function for a single x ###
def f_single(x, J, beta):
    diff = x[:, None] == x[None, :]
    energy = np.sum(J * diff) / 2
    return np.exp(-beta * energy)

def f_parallel(X, J, beta):
    with Pool(processes=cpu_count()) as pool:
        results = pool.starmap(f_single, [(x, J, beta) for x in X])
    return np.array(results)

### 3. Kernel mean embedding & Gram matrix ###
def kernel_embedding(lambda_, d):
    return np.exp(-lambda_ * d / 2) * (np.cosh(lambda_ / 2) ** d)
...
def gram_matrix(X, lambda_, d):
    ...
    return np.exp(-lambda_ * 0.5 * (d - inner))

def kernel_vec(x, X, lambda_, d):
    x_rescaled = 2 * x - 1.
    X_rescaled = 2 * X - 1.
    inner = X_rescaled @ x_rescaled
    return np.exp(-lambda_ * 0.5 * (d - inner)).reshape(-1, 1)
...
def bayesian_cubature(X, f_vals, lambda_, d):
    ...
    var = double_integral(lambda_, d) - (z.T @ K_inv_z).item()
    return mean, var
...
def compute_integral_variance(X, lambda_, d):
    ...
    L, lower = cho_factor(K)
    K_inv_z = cho_solve((L, lower), z)
    return double_integral(lambda_, d) - (z.T @ K_inv_z).item()

def compute_augmented_variance(X_obs, y_obs, x, lambda_, d):
...
def compute_kg(X_obs, y_obs, candidates, lambda_, d, num_samples):
    ...
    for x in candidates:
        ...
        updated_vars = Parallel(n_jobs=-1)(
            delayed(compute_augmented_variance)(X_obs, y_obs, x, lambda_, d)
...
def run_experiment(f, n_vals, lambda_, d, L, seed, use_bo, num_mc):
    ...
    true_vals = f_parallel(all_states, J, beta)
    true_expectation = np.mean(true_vals)
...
### 7. Plotting ###
def plot_results(n_vals, results):
    bmc_errors = np.abs(results["bmc_means"] - results["true_val"])
    mc_errors = np.abs(results["mc_means"] - results["true_val"])
    bmc_errors = np.clip(bmc_errors, 1e-10, None)
    mc_errors = np.clip(mc_errors, 1e-10, None)

    plt.figure(figsize=(10, 6))
    plt.plot(n_vals, mc_errors, 'ko-', label="MC Error")
    plt.plot(n_vals, bmc_errors, 'ro-', label="BQ Error")
    plt.yscale('log')
    plt.xlabel("n")
    plt.ylabel("Abs Error (log)")
    plt.title("Bayesian Quadrature vs Monte Carlo")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("results/bq_vs_mc.png")
    plt.close()
...
def main():
    ...
    results = run_experiment(f_single, n_vals, lambda_, d, L, seed, use_bo, num_mc)
    # print("BMC means:", results["bmc_means"])
    # print("MC means:", results["mc_means"])
